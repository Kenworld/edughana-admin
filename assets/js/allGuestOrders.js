import { fetchOrders, fetchOrderById } from "./Orders.js";
...
async function renderOrdersTable() {
  const orders = (await fetchOrders(true)).map((o) => ({
    ...o,
    isGuest: true,
  }));
...
async function showOrderDetails(orderId, isGuest) {
  ...
  if (!order) return;
  order.isGuest = true; // Ensure the modal always shows Guest for this page
  currentOrder = { id: order.id, isGuest: true }; // Store current order info
            ...
            ${order.items
              ?.map(
                ...
                <li>
                  ...
                  item.salePrice
                    ? ` | <strong>Sale:</strong> ${formatCurrency(
                        item.salePrice
                      )}`
                    : ""
                }
...
async function handleUpdateStatus(status) {
  ...
  try {
    ...
    modalInstance.hide();
    await renderOrdersTable(); // Refresh the table
  } catch (error) {
